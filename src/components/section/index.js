...
const Section = ({ title, subTitle, children, margin, padding, bgColor, flexRow,  flexSpacing, marginTop, sectionType,wave, extras}) => {
  ...
  const bgStyle = bgColor === "primary" ? "bg-cogengreen" : bgColor === "light-green" ? "bg-green-100"  : bgColor === "white" ? "bg-white" : bgColor === "lightGrey" ?  "xxs:bg-gray-100 " :  bgColor === "lightGray" ?  "bg-gray-100 " : "";
...
if(sectionType == "fluid")
  ...
  return (
    ...
    >
      ...
      <h1 className="text-cogengreen text-lg font-poppins tracking-lg  uppercase mb-20">{extras && extras}</h1>
        {title && title}
      </h1>
      
      <p className="text-lg font-poppins my-8">{subTitle && subTitle}</p>
      <div className={`flex align-center  md:flex-row sm:flex-col xxs:flex-col my-8  font-poppins ${flexRows} ${spacing} xxs:text-center` }>
